...
import { renderHtmlStr } from '#/render-html-str2';

function renderHtml<T extends typeof Store>(Ctor: T) {
  return renderHtmlStr(Ctor as any);
}
...
describe('renderHtmlStr — attributes', () => {
  ...
  it('should render class from a single-level array', () => {
    class App extends Store {
      cls = ['btn primary', { active: true, hidden: false }, ['nested'], 123];
      ui = bobe`div class={cls} children="hi"`;
    }
    expect(renderHtml(App)).toBe('<div class="btn primary active">hi</div>');
  });
...
describe('renderHtmlStr — text and html', () => {
  ...
  it('should skip indented children when html prop is set (children from parseHtmlToFibers)', () => {
    class App extends Store {
      ui = bobe`
        div html="<b>bold</b>"
          span children="child"
    ...
    // inset 子节点被 beforeIndent 跳过，html 内容来自 parseHtmlToFibers 解析
    expect(renderHtml(App)).toBe('<div><b>bold</b></div>');
  });
});
...
describe('renderHtmlStr — tp (teleport)', () => {
  it('should render tp content into target when target node is before tp (SSR sync render)', () => {
    class App extends Store {
      myBlock: any = null;
      ui = bobe`
        div ref={myBlock}
    ...
    expect(renderHtml(App)).toBe(
      '<div><!--tp-content-before--><span>teleported</span><!--tp-content-after--></div><!--tp-after-->'
    );
  });

  it('should skip tp content during SSR when target node is after tp (async effect insertion)', () => {
    class App extends Store {
      myBlock: any = null;
      ui = bobe`
        tp node={myBlock}
