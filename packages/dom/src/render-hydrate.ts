...
class TreeCursor {
  ...
  tryClaimComment(name: string): Comment | null {
    return this.findUnclaimed(node => node.nodeType === 8 && (node as Comment).data === name) as Comment | null;
  }

  /** 认领一个未使用的文本节点（非空文本） */
  tryClaimText(): Text | null {
    return this.findUnclaimed(node => node.nodeType === 3 && !!node.textContent?.trim()) as Text | null;
  }
}
...
export const hydrate = (ComponentClass: typeof Store, rootEl: Element) => {
  ...
  // 首屏 program 遍历期间为 true。onBeforeFlush 之后进入普通更新阶段。
  let isFirstRender = true;

  // 已认领的 SSR 节点内容由服务端生成，首屏只 seed 状态，跳过写 DOM 避免重绘。
  ...
  function createNode(this: Interpreter, name: string): Node {
    ...
    return name === 'text' ? doc.createTextNode('') : browserCreateNode.call(this, name);
  }
  ...
  const render = customRender({
    ...
    leaveNode,
    leaveLogicNode,
    // program() 遍历完模板后切回普通更新阶段。
    // 后续更新中的 createNode/createAnchor 走浏览器新建路径，不再尝试从 SSR DOM 认领。
    onBeforeFlush: () => {
      isFirstRender = false;
    }
  });
