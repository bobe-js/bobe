import { SignalNode as Signal } from '#/index';
...
export class DepStr2 {
  ...
  depIs(str: string) {
    ...
    for (const key in this.signals) {
      const curr = this.signals[key] as Signal;
      const currName = this.reflect.get(curr);
      let line = curr['emitHead'];
      while (line != null) {
        // if (line.down && line.down === line.down['scope']) continue;
        const downName = this.reflect.get(line.down);
        state.push(`${currName} -> ${downName}`);
        line = line.nextEmitLine;
      }
      line = curr['recHead'];
      while (line != null) {
        const upName = this.reflect.get(line.up);
        state.push(`${upName} <- ${currName}`);
        line = line.nextRecLine;
      }
    }
  ...
  outLinkIs({ ins }: { ins: Signal }, outLink: string) {
    ...
    const hasLinks = [];
    while (point) {
      const refedName = this.reflect.get(point['up']);
      hasLinks.push(refedName);
      point = point.nextOutLink;
    }
    hasLinks.sort();
